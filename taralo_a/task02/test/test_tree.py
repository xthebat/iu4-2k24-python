...
import pytest
from src.main_tree import tree
...
def test_tree(create_temporary_directory) -> None:
    ...
    result = tree(path_name =directory_path, result = [], level=-1, lvl_mod=False)
    expected_result = [
        "└── root",
        "    ├── subdir2",
        "    │   ├── file3.txt",
...
def test_tree_with_level_mod(create_temporary_directory):
    ...
    directory_path = create_temporary_directory
    result = tree(path_name = directory_path, result = [], level=2, lvl_mod=True)
    expected_result = [
        "└── root",
        "    ├── subdir2",
        "    ├── subdir1",
