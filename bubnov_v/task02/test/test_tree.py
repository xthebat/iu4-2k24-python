...
from task02.src.tree import create_tree, output_tree
...
def test_output_tree(dir_name: str, level: int, file_answer: str, structure: list):
    ...
    # Remove all files and directories, when created
    os.remove(os.path.join(os.path.dirname(os.path.realpath(__file__)), 'file_output.txt'))
    remove_structure(dir_path, structure)
